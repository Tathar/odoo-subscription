...
class report_subscription(osv.osv):
    ...
    _columns = {
        ...
        'date_to': fields.date('End Date', readonly=True, help="End subscription date"),
        ...
        'subscription_id': fields.many2one('product.product', 'Subscription Product', readonly=True),
        'subscription_state': fields.selection(STATE, 'Current Subscription State', readonly=True),
    ...
    def init(self, cr):
        '''Create the view'''
        tools.drop_view_if_exists(cr, 'report_subscription')
        cr.execute("""
        CREATE OR REPLACE VIEW report_subscription AS (
        SELECT
        MIN(id) AS id,
        partner_id,
        count(subscription_id) as quantity,
        user_id,
        subscription_state,
        associate_member_id,
        subscription_amount,
        ...
        (SELECT
            ...
            p.subscription_state AS subscription_state,
            p.associate_member AS associate_member_id,
            p.subscription_amount AS subscription_amount,
            p.subscription_stop AS date_to,
            ...
            CASE WHEN ml.state = 'paid' OR p.subscription_state = 'old' THEN SUM(il.price_subtotal) ELSE 0 END AS tot_earned,
            ml.subscription_id AS subscription_id,
            p.company_id AS company_id
            FROM res_partner p
            LEFT JOIN subscription_subscription_line ml ON (ml.partner = p.id)
            LEFT JOIN account_invoice_line il ON (ml.account_invoice_line = il.id)
            LEFT JOIN account_invoice ai ON (il.invoice_id = ai.id)
            WHERE p.subscription_state != 'none' and p.active = 'true'
            GROUP BY
              p.id,
              p.user_id,
