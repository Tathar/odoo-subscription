...
class subscription_invoice(osv.osv_memory):
    """subscription Invoice"""

    _name = "subscription.invoice"
    _description = "subscription Invoice"
    _columns = {
        'product_id': fields.many2one('product.product','subscription', required=True),
    ...
    def subscription_invoice(self, cr, uid, ids, context=None):
        ...
        if data:
            data = data[0]
            datas = {
                'subscription_product_id': data.product_id.id,
                'amount': data.member_price
            }
        invoice_list = partner_obj.create_subscription_invoice(cr, uid, context.get('active_ids', []), datas=datas, context=context)
